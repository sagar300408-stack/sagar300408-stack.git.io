-- ==========================================
-- 004: Setup Wizard & System Initialization
-- ==========================================

-- 0. Fix user_roles NULL uniqueness
-- PostgreSQL UNIQUE constraints do not treat NULL = NULL, so
-- ON CONFLICT won't fire for org-wide roles (workspace_id IS NULL).
CREATE UNIQUE INDEX IF NOT EXISTS user_roles_org_wide_unique_idx
  ON public.user_roles (user_id, org_id)
  WHERE workspace_id IS NULL;

-- 1. System Settings Table (Global, not org-scoped)
-- Used as the authoritative initialization flag.
...
-- Anyone authenticated can READ system settings (needed for bootstrap check)
CREATE POLICY "Authenticated users can read system settings" ON public.system_settings
  FOR SELECT USING (auth.role() = 'authenticated');

-- Only owners can write system settings (enforced via SECURITY DEFINER RPC)
...
-- Anonymous also needs to read system settings to know if CMS is initialized
-- before they can even log in. Expose only cms_initialized key.
CREATE POLICY "Anonymous can read initialization flag" ON public.system_settings
  FOR SELECT USING (key = 'cms_initialized');


-- ==========================================
-- 2. Fix RLS Recursion on user_roles
-- ==========================================
-- The original policy had: EXISTS (SELECT 1 FROM public.user_roles ...)
-- inside a user_roles policy, causing infinite recursion.
...
-- Non-recursive helper: is current user a member of an org?
...
-- Recreate user_roles policies using non-recursive helpers
...
-- Allow INSERT of own role during initialization (handled via SECURITY DEFINER RPC)
-- The initialize_cms RPC below bypasses RLS entirely.


-- ==========================================
-- 3. get_system_status() RPC
-- Returns the full bootstrap state in a single call.
-- ==========================================
...
-- ==========================================
-- 4. initialize_cms() RPC
-- Called once by the Setup Wizard.
-- Uses pg_advisory_xact_lock to prevent concurrent initialization.
-- Fully idempotent via UPSERTs.
-- ==========================================
...
BEGIN
  -- Advisory lock to prevent race conditions (two browsers clicking simultaneously)
  ...
  -- Get calling user (never hardcoded)
  _calling_user := auth.uid();
  IF _calling_user IS NULL THEN
    RAISE EXCEPTION 'Authentication required. You must be signed in to initialize the CMS.';
  END IF;
  ...
  ON CONFLICT (org_id, slug) DO UPDATE SET name = EXCLUDED.name
  RETURNING id INTO _casestudies_type_id;
  ...
  ON CONFLICT (org_id, slug) DO UPDATE SET name = EXCLUDED.name
  RETURNING id INTO _docs_type_id;

  -- ── Default Categories for Insights ───────────────────────────
  ...
  -- Assign Owner Role (uses partial index for NULL workspace_id)
